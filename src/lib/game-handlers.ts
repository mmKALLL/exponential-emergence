...
function resetAction(action: Action) {
  ...
  action.bestValueHistory = didImproveBest
    ? action.valueHistory
    : action.bestValueHistory;
}
...
function handleGoalCompletion() {
  ...
  const currentAmount =
    gs.levels[gs.currentLevel].resources[currentGoal.resourceName];
...
export const Game = {
  ...
  get resources() {
    return Object.entries(gs.levels[gs.currentLevel].resources).map(
  ...
  get currentGoalAmount() {
    return Game.currentGoal
      ? gs.levels[gs.currentLevel].resources[Game.currentGoal.resourceName]
      : null;
  },
  ...
  toggleAction(action: Action) {
    ...
    gs.currentActionName =
      gs.currentActionName !== action.name ? action.name : null;
  },
  ...
  gameTick: () => {
    ...
    if (gs.currentActionName) {
      const action =
        gs.levels[gs.currentLevel].actionCards[gs.currentActionName];
